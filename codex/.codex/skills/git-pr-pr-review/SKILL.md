...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$git-pr-pr-review`.
- Treat all user text after `$git-pr-pr-review` as `{{SC_ARGS}}`.
...
- Original command: `git:pr:pr-review`.
- Codex invocation: `$git-pr-pr-review`.
...
- PARSE arguments from {{SC_ARGS}} for action and PR number
- SET session data:
  ```json
  {
...
ELSE IF {{SC_ARGS}} starts with action keyword (approve|comment|request-changes|diff|checks|files):

- EXTRACT action and optional PR number
- SET review_action = detected action

ELSE IF {{SC_ARGS}} is numeric:

- SET prNumber = {{SC_ARGS}}
...
cat > /tmp/pr-review-$SESSION_ID/performance-metrics.json << EOF
{
  ...
  "sequentialEstimate": {
    ...
    "tasksIfSequential": 8,
    "averagespawn_agentTime": "8-10 seconds"
  }
}
EOF
```
