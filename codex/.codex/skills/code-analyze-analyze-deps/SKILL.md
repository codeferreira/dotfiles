---
name: "code-analyze-analyze-deps"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-analyze-analyze-deps`.
- Treat all user text after `$code-analyze-analyze-deps` as `{{SC_ARGS}}`.
...
- Original command: `code:analyze:analyze-deps`.
- Codex invocation: `$code-analyze-analyze-deps`.
...
Analyze project dependencies using modern tools. Analysis type: {{SC_ARGS}} (security, outdated, size, licenses, graph, or all - defaults to 'all')
...
- Parse analysis type from {{SC_ARGS}} (default: 'all')
