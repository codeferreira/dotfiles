---
name: "agent-merge-agent-prep-merge"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$agent-merge-agent-prep-merge`.
- Treat all user text after `$agent-merge-agent-prep-merge` as `{{SC_ARGS}}`.
...
- Original command: `agent:merge:agent-prep-merge`.
- Codex invocation: `$agent-merge-agent-prep-merge`.
...
IF ({{SC_ARGS}}.includes("--all-worktrees")) {
  options.allWorktrees = true;
} ELSE IF ({{SC_ARGS}}.includes("--dry-run")) {
  options.dryRun = true;
} ELSE IF ({{SC_ARGS}}.includes("--create-prs")) {
  options.createPRs = true;
} ELSE IF ({{SC_ARGS}} && !{{SC_ARGS}}.startsWith("--")) {
  targetBranches = {{SC_ARGS}}.split(" ");
} ELSE {
