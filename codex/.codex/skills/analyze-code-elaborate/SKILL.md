...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$analyze-code-elaborate`.
- Treat all user text after `$analyze-code-elaborate` as `{{SC_ARGS}}`.
...
- Original command: `analyze:code:elaborate`.
- Codex invocation: `$analyze-code-elaborate`.
...
**IMMEDIATELY DEPLOY 10 PARALLEL ANALYSIS AGENTS** for ultra-fast comprehensive technical analysis: **{{SC_ARGS}}**
