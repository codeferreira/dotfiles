...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$kubernetes-k8s-debug`.
- Treat all user text after `$kubernetes-k8s-debug` as `{{SC_ARGS}}`.
...
- Original command: `kubernetes:k8s-debug`.
- Codex invocation: `$kubernetes-k8s-debug`.
...
INPUT: resource_name = {{SC_ARGS}} || null
