...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$git-pr-pr-create`.
- Treat all user text after `$git-pr-pr-create` as `{{SC_ARGS}}`.
...
- Original command: `git:pr:pr-create`.
- Codex invocation: `$git-pr-pr-create`.
...
- PARSE {{SC_ARGS}} for PR configuration options
...
draft_mode=$(echo "{{SC_ARGS}}" | grep -q "draft" && echo "true" || echo "false")
base_branch=$(echo "{{SC_ARGS}}" | grep -oE 'base=([^\s]+)' | cut -d'=' -f2 || echo "main")
reviewers=$(echo "{{SC_ARGS}}" | grep -oE 'reviewers=([^\s]+)' | cut -d'=' -f2 || echo "")
labels=$(echo "{{SC_ARGS}}" | grep -oE 'labels=([^\s]+)' | cut -d'=' -f2 || echo "")
custom_title=$(echo "{{SC_ARGS}}" | grep -oE 'title="([^"]+)"' | sed 's/title="//;s/"$//' || echo "")
template=$(echo "{{SC_ARGS}}" | grep -oE 'template=([^\s]+)' | cut -d'=' -f2 || echo "")
```
