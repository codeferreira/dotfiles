...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$context-project-auto`.
- Treat all user text after `$context-project-auto` as `{{SC_ARGS}}`.
...
- Original command: `context:project:auto`.
- Codex invocation: `$context-project-auto`.
...
**IMMEDIATELY DEPLOY 8 PARALLEL CONTEXT-LOADING AGENTS** for ultra-fast comprehensive project context discovery: **{{SC_ARGS}}** (optional focus area)
...
Generate update_plan list based on discovered context and project state
...
6. **.codex/** directory - Project-specific Claude configurations
