...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$test-run-load-test`.
- Treat all user text after `$test-run-load-test` as `{{SC_ARGS}}`.
...
- Original command: `test:run:load-test`.
- Codex invocation: `$test-run-load-test`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Load test target: {{SC_ARGS}}
