...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$analyze-data-data-flow`.
- Treat all user text after `$analyze-data-data-flow` as `{{SC_ARGS}}`.
...
- Original command: `analyze:data:data-flow`.
- Codex invocation: `$analyze-data-data-flow`.
...
IF {{SC_ARGS}} contains specific source/destination:
