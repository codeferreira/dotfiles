...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$security-audit-audit`.
- Treat all user text after `$security-audit-audit` as `{{SC_ARGS}}`.
...
- Original command: `security:audit:audit`.
- Codex invocation: `$security-audit-audit`.
