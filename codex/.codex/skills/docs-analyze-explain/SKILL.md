...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$docs-analyze-explain`.
- Treat all user text after `$docs-analyze-explain` as `{{SC_ARGS}}`.
...
- Original command: `docs:analyze:explain`.
- Codex invocation: `$docs-analyze-explain`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Explanation target: {{SC_ARGS}}
...
- ANALYZE target type from {{SC_ARGS}}:
...
- USE spawn_agent tool for comprehensive research if complex:
