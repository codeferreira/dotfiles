...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$tool-diagram`.
- Treat all user text after `$tool-diagram` as `{{SC_ARGS}}`.
...
- Original command: `tool:diagram`.
- Codex invocation: `$tool-diagram`.
...
- Extract diagram type from {{SC_ARGS}} (default: architecture)
