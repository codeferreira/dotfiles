...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$git-pr-pr-update`.
- Treat all user text after `$git-pr-pr-update` as `{{SC_ARGS}}`.
...
- Original command: `git:pr:pr-update`.
- Codex invocation: `$git-pr-pr-update`.
...
FOR EACH update request in {{SC_ARGS}}:
