...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-migrate-deno-ify`.
- Treat all user text after `$code-migrate-deno-ify` as `{{SC_ARGS}}`.
...
- Original command: `code:migrate:deno-ify`.
- Codex invocation: `$code-migrate-deno-ify`.
