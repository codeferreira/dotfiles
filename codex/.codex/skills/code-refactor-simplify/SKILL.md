...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-refactor-simplify`.
- Treat all user text after `$code-refactor-simplify` as `{{SC_ARGS}}`.
...
- Original command: `code:refactor:simplify`.
- Codex invocation: `$code-refactor-simplify`.
...
IF {{SC_ARGS}} specifies file or directory:
