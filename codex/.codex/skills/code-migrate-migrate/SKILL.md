...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-migrate-migrate`.
- Treat all user text after `$code-migrate-migrate` as `{{SC_ARGS}}`.
...
- Original command: `code:migrate:migrate`.
- Codex invocation: `$code-migrate-migrate`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Migration target: {{SC_ARGS}}
...
ELSE IF {{SC_ARGS}} contains "api" OR "version":
...
ELSE IF {{SC_ARGS}} contains "dependency" OR "update":
...
ELSE IF {{SC_ARGS}} contains "kubernetes" OR "k8s":
