...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-improve`.
- Treat all user text after `$sc-improve` as `{{SC_ARGS}}`.
...
- Original command: `improve`.
- Codex invocation: `$sc-improve`.
...
- **spawn_agent**: Delegation for large-scale improvement workflows requiring systematic coordination
