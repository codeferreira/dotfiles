---
name: "meta-command-generate-command"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$meta-command-generate-command`.
- Treat all user text after `$meta-command-generate-command` as `{{SC_ARGS}}`.
...
- Original command: `meta:command:generate-command`.
- Codex invocation: `$meta-command-generate-command`.
...
- **Agent 2: Existing Command Pattern Analyzer**: Analyze existing .codex/commands structure and patterns
...
3. **Input Pattern & State Management**:
   ...
   - **Single argument**: Use `{{SC_ARGS}}` token with validation
...
4. **Execution Model (CRITICAL for performance)**:
   ...
   - **Parallel Sub-Agent**: Benefits from spawn_agent tool delegation (5-10x faster)
...
6. **Security & Tool Permissions** (Minimal Principle):
   ...
   - **Sub-processes**: spawn_agent (for parallel execution)
...
```yaml
---
allowed-tools: spawn_agent, [minimal_other_tools]
...
- [ ] **Sub-Agent Opportunities**: Analysis tasks use spawn_agent tool for parallelism
...
namespace_path=".codex/commands/${namespace_directory}"
...
```yaml
---
allowed-tools: spawn_agent, Read, Bash(rg:*), Bash(fd:*), Bash(gdate:*)
