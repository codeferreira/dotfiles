...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$docs-generate-document`.
- Treat all user text after `$docs-generate-document` as `{{SC_ARGS}}`.
...
- Original command: `docs:generate:document`.
- Codex invocation: `$docs-generate-document`.
...
- CREATE inter-agent communication directory: `/tmp/doc-agents-$SESSION_ID/`
  ELSE IF {{SC_ARGS}} contains specific flags (--readme-only, --changelog-only, etc.):
