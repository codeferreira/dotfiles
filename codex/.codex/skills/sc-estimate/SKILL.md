...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-estimate`.
- Treat all user text after `$sc-estimate` as `{{SC_ARGS}}`.
...
- Original command: `estimate`.
- Codex invocation: `$sc-estimate`.
...
- **spawn_agent**: Advanced delegation for multi-domain estimation requiring systematic coordination
