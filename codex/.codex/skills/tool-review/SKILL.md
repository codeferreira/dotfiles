...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$tool-review`.
- Treat all user text after `$tool-review` as `{{SC_ARGS}}`.
...
- Original command: `tool:review`.
- Codex invocation: `$tool-review`.
...
- Load target files from {{SC_ARGS}} or analyze entire project if no specific targets
