...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$test-fix-flaky-fix`.
- Treat all user text after `$test-fix-flaky-fix` as `{{SC_ARGS}}`.
...
- Original command: `test:fix:flaky-fix`.
- Codex invocation: `$test-fix-flaky-fix`.
...
- DETERMINE scope: specific test vs. project-wide analysis based on {{SC_ARGS}}
...
export class TestIsolation {
  private static cleanupspawn_agents: (() => Promise<void>)[] = [];

  static addCleanup(task: () => Promise<void>): void {
    this.cleanupspawn_agents.push(task);
  }

  static async cleanup(): Promise<void> {
    for (const task of this.cleanupspawn_agents) {
      try {
        await task();
      } catch (error) {
    ...
    this.cleanupspawn_agents.length = 0;
  }
