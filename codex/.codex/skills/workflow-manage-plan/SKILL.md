...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$workflow-manage-plan`.
- Treat all user text after `$workflow-manage-plan` as `{{SC_ARGS}}`.
...
- Original command: `workflow:manage:plan`.
- Codex invocation: `$workflow-manage-plan`.
...
**Enterprise-Scale spawn_agent Categories:**

- **Foundation spawn_agents** (Sequential, High Priority):
...
**Startup-Scale spawn_agent Categories:**
...
STEP 7: Automatic update_plan integration with hierarchical progress tracking

**Intelligent Todo Synchronization:**

```bash
# STEP 7.1: Create hierarchical todo structure
echo "📝 Synchronizing with update_plan system..."
...
**spawn_agent Management Command Integration:**
...
echo "  📋 Total spawn_agents: $total_tasks"
