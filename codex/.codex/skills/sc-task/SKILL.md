...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-task`.
- Treat all user text after `$sc-task` as `{{SC_ARGS}}`.
...
- Codex invocation: `$sc-task`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

# $sc-task - Enhanced spawn_agent Management
...
- spawn_agents benefiting from systematic execution and progressive enhancement
...
- **spawn_agent**: Advanced delegation for complex multi-agent coordination and sub-task management
- **Read/Write/Edit**: spawn_agent documentation and implementation coordination
- **sequentialthinking**: Structured reasoning for complex task dependency analysis

## Key Patterns
- **spawn_agent Hierarchy**: Epic-level objectives → Story coordination → spawn_agent execution → Subtask granularity
...
- **Cross-Session Management**: spawn_agent persistence → context continuity → progressive enhancement
...
**Completion Criteria**:
- spawn_agent objective achieved
- All sub-tasks marked completed in update_plan
- Validation passed

**Output**: spawn_agent completion report with:
