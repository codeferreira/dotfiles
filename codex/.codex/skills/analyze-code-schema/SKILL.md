...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$analyze-code-schema`.
- Treat all user text after `$analyze-code-schema` as `{{SC_ARGS}}`.
...
- Original command: `analyze:code:schema`.
- Codex invocation: `$analyze-code-schema`.
