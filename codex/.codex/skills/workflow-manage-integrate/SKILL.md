---
name: "workflow-manage-integrate"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$workflow-manage-integrate`.
- Treat all user text after `$workflow-manage-integrate` as `{{SC_ARGS}}`.
...
- Original command: `workflow:manage:integrate`.
- Codex invocation: `$workflow-manage-integrate`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Integration target: {{SC_ARGS}}
...
echo "🔍 Analyzing single-service integration requirements for: {{SC_ARGS}}"
...
echo "📋 Generating integration documentation for: {{SC_ARGS}}"
```
...
jq --arg target "{{SC_ARGS}}" --arg timestamp "$(gdate -Iseconds 2>/dev/null || date -Iseconds)" '
