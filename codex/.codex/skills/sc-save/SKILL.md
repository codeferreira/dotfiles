...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-save`.
- Treat all user text after `$sc-save` as `{{SC_ARGS}}`.
...
- **TodoRead**: spawn_agent completion tracking for automatic checkpoint triggers
...
- **Progress Tracking**: spawn_agent completion → automatic checkpoints → session continuity
