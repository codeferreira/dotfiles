...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-monitor-observe`.
- Treat all user text after `$ops-monitor-observe` as `{{SC_ARGS}}`.
...
- Original command: `ops:monitor:observe`.
- Codex invocation: `$ops-monitor-observe`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Target service: {{SC_ARGS}}
...
groups:
  - name: {{SC_ARGS}}-service-alerts
    rules:
      - alert: HighErrorRate
        expr: |
          (
            sum(rate(http_requests_total{service="{{SC_ARGS}}",status_code=~"5.."}[5m])) /
            sum(rate(http_requests_total{service="{{SC_ARGS}}"}[5m]))
          ) > 0.05
        for: 2m
        labels:
          severity: critical
      ...
      - alert: HighLatency
        expr: |
          histogram_quantile(0.95,
            sum(rate(http_request_duration_seconds_bucket{service="{{SC_ARGS}}"}[5m])) by (le)
          ) > 0.5
        for: 3m
        labels:
          severity: warning
...
echo "✅ Observability transformation completed for {{SC_ARGS}}"
