...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$test-run-tdd`.
- Treat all user text after `$test-run-tdd` as `{{SC_ARGS}}`.
...
- Original command: `test:run:tdd`.
- Codex invocation: `$test-run-tdd`.
...
FOR target_component IN {{SC_ARGS}}:
