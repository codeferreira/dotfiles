...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$meta-reflect-reflection`.
- Treat all user text after `$meta-reflect-reflection` as `{{SC_ARGS}}`.
...
- Original command: `meta:reflect:reflection`.
- Codex invocation: `$meta-reflect-reflection`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Reflection scope: {{SC_ARGS}}
...
```json
{
  "period_scope": "{{SC_ARGS}} or current session",
