---
name: "scaffold-rust-scaffold-rust-cli"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$scaffold-rust-scaffold-rust-cli`.
- Treat all user text after `$scaffold-rust-scaffold-rust-cli` as `{{SC_ARGS}}`.
...
- Original command: `scaffold:rust:scaffold-rust-cli`.
- Codex invocation: `$scaffold-rust-scaffold-rust-cli`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Target CLI name: {{SC_ARGS}}
...
- CREATE new Cargo project: `cargo new {{SC_ARGS}} --bin`
...
repository = "https://github.com/username/{{SC_ARGS}}"
readme = "README.md"
...
- `{{SC_ARGS}} init` - Initialize configuration
- `{{SC_ARGS}} status` - Show current status
...
Configuration file location: `~/.config/{{SC_ARGS}}/config.toml`
...
fd . {{SC_ARGS}} -t d | head -10

echo "\n📄 Key files created:"
fd . {{SC_ARGS}} -t f | rg '\.(rs|toml|md)$' | head -10

echo "\n🧪 Run initial tests:"
cd {{SC_ARGS}} && cargo test

echo "\n🚀 Build and run:"
cd {{SC_ARGS}} && cargo run -- --help
````

**Development Workflow Recommendations:**

1. **Start Development**: `cd {{SC_ARGS}} && cargo run -- --help`
