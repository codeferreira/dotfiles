---
name: "scaffold-rust-scaffold-rust-axum"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$scaffold-rust-scaffold-rust-axum`.
- Treat all user text after `$scaffold-rust-scaffold-rust-axum` as `{{SC_ARGS}}`.
...
- Original command: `scaffold:rust:scaffold-rust-axum`.
- Codex invocation: `$scaffold-rust-scaffold-rust-axum`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Target project name: {{SC_ARGS}}
...
repository = "https://github.com/yourusername/{{SC_ARGS}}"
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "{{SC_ARGS}}=debug,tower_http=debug".into()),
        )
...
pub struct AppConfig {
    ...
    #[envconfig(from = "DATABASE_URL", default = "postgres://localhost/{{SC_ARGS}}")]
...
async fn test_health_endpoint() {
    let app = {{SC_ARGS}}::create_router({{SC_ARGS}}::AppState {
        config: {{SC_ARGS}}::config::AppConfig::from_env().unwrap(),
    });
...
async fn test_root_endpoint() {
    ...
    response.assert_status_ok();
    response.assert_text("Hello, {{SC_ARGS}}!");
}
```
