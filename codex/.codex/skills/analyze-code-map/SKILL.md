...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$analyze-code-map`.
- Treat all user text after `$analyze-code-map` as `{{SC_ARGS}}`.
...
- Original command: `analyze:code:map`.
- Codex invocation: `$analyze-code-map`.
...
IF {{SC_ARGS}} is vague or incomplete:
