...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-analyze-deps`.
- Treat all user text after `$code-analyze-deps` as `{{SC_ARGS}}`.
...
- Original command: `code:analyze:deps`.
- Codex invocation: `$code-analyze-deps`.
