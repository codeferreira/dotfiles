...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-deploy-containerize`.
- Treat all user text after `$ops-deploy-containerize` as `{{SC_ARGS}}`.
...
- Original command: `ops:deploy:containerize`.
- Codex invocation: `$ops-deploy-containerize`.
...
```bash
echo "✅ Containerization completed for: {{SC_ARGS}}"
