...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-fedora-fedora-expert`.
- Treat all user text after `$ops-fedora-fedora-expert` as `{{SC_ARGS}}`.
...
- Original command: `ops:fedora:fedora-expert`.
- Codex invocation: `$ops-fedora-fedora-expert`.
