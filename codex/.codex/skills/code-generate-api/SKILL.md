...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-generate-api`.
- Treat all user text after `$code-generate-api` as `{{SC_ARGS}}`.
...
- Original command: `code:generate:api`.
- Codex invocation: `$code-generate-api`.
...
Generate comprehensive API endpoint for {{SC_ARGS}} using intelligent framework detection, parallel implementation, and extensive testing.
