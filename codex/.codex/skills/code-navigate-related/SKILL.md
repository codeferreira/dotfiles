...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-navigate-related`.
- Treat all user text after `$code-navigate-related` as `{{SC_ARGS}}`.
...
- Original command: `code:navigate:related`.
- Codex invocation: `$code-navigate-related`.
