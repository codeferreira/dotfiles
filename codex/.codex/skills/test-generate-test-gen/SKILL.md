...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$test-generate-test-gen`.
- Treat all user text after `$test-generate-test-gen` as `{{SC_ARGS}}`.
...
- Original command: `test:generate:test-gen`.
- Codex invocation: `$test-generate-test-gen`.
...
```typescript
// Generated comprehensive tests for {{SC_ARGS}}
import { assertEquals, assertExists, assertRejects } from "@std/assert";
import { {{SC_ARGS}} } from "../src/{{SC_ARGS}}.ts";
...
// Happy path tests
Deno.test("{{SC_ARGS}} - successful operations with valid data", async () => {
  const service = new {{SC_ARGS}}();
...
Deno.test("{{SC_ARGS}} - validation errors with invalid data", async () => {
  const service = new {{SC_ARGS}}();
...
// Edge case tests
Deno.test("{{SC_ARGS}} - edge cases and boundary conditions", async () => {
  const service = new {{SC_ARGS}}();
...
Deno.test("{{SC_ARGS}} - performance with multiple operations", async () => {
  const service = new {{SC_ARGS}}();
  const operations = [];
...
```rust
// Generated comprehensive tests for {{SC_ARGS}} module
#[cfg(test)]
mod tests {
    use super::*;
    ...
    async fn test_concurrent_operations() {
        let service = std::sync::Arc::new({{SC_ARGS}}::new());
        let mut handles = vec![];
...
echo "🔍 Discovering API endpoints for {{SC_ARGS}}..."
API_ENDPOINTS=$(rg "(GET|POST|PUT|DELETE|PATCH).*{{SC_ARGS}}" --type-add 'web:*.{js,ts,rs,go,java}' --type web 2>/dev/null | head -10 || echo "No API endpoints found")
...
jq --arg timestamp "$(gdate -Iseconds 2>/dev/null || date -Iseconds)" --arg target "{{SC_ARGS}}" '
...
```bash
echo "✅ Test generation completed for {{SC_ARGS}}"
