...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-analyze-bottleneck`.
- Treat all user text after `$code-analyze-bottleneck` as `{{SC_ARGS}}`.
...
- Original command: `code:analyze:bottleneck`.
- Codex invocation: `$code-analyze-bottleneck`.
...
**IMMEDIATELY DEPLOY 10 PARALLEL PERFORMANCE ANALYSIS AGENTS** for ultra-fast comprehensive bottleneck analysis: **{{SC_ARGS}}**
