...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$security-model-harden`.
- Treat all user text after `$security-model-harden` as `{{SC_ARGS}}`.
...
- Original command: `security:model:harden`.
- Codex invocation: `$security-model-harden`.
