...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-business-panel`.
- Treat all user text after `$sc-business-panel` as `{{SC_ARGS}}`.
...
- Original command: `sc:business-panel`.
- Codex invocation: `$sc-business-panel`.
...
- **Tool Orchestration**: Read, Grep, Write, MultiEdit, update_plan
