...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$workflow-view-progress`.
- Treat all user text after `$workflow-view-progress` as `{{SC_ARGS}}`.
...
- Original command: `workflow:view:progress`.
- Codex invocation: `$workflow-view-progress`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Progress target: {{SC_ARGS}}
...
CASE operation_type:
WHEN "record_progress" (default if {{SC_ARGS}} contains text):
...
WHEN "view_progress" ({{SC_ARGS}} contains --today, --date, --all, --recent):

**Progress Viewing Operations:**

```bash
if [[ "{{SC_ARGS}}" == *"--today"* ]]; then
    current_date=$(gdate +%Y-%m-%d 2>/dev/null || date +%Y-%m-%d)
...
WHEN "search_progress" ({{SC_ARGS}} contains search terms):

**Progress Search Operations:**

```bash
echo "🔍 Searching progress entries for: {{SC_ARGS}}"
if command -v rg >/dev/null 2>&1; then
    rg "{{SC_ARGS}}" ./progress/ --type md --context 2 --color always 2>/dev/null
else
    fd "\.md$" ./progress/ 2>/dev/null | xargs grep -l "{{SC_ARGS}}" | while read -r file; do
        echo "\n📄 Found in: $file"
        grep -n "{{SC_ARGS}}" "$file" 2>/dev/null
    done
fi
```
...
if [[ ! -w "$(pwd)" ]]; then
    ...
    echo "$(gdate -Iseconds 2>/dev/null || date -Iseconds): {{SC_ARGS}}" >> ./progress-fallback.txt
...
jq --arg entry "{{SC_ARGS}}" --arg file "$progress_file" --arg timestamp "$(gdate -Iseconds 2>/dev/null || date -Iseconds)" '
  .progressEntry = $entry |
  .progressFile = $file |
