---
name: "meta-extract-knowledge-extract"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$meta-extract-knowledge-extract`.
- Treat all user text after `$meta-extract-knowledge-extract` as `{{SC_ARGS}}`.
...
- Original command: `meta:extract:knowledge-extract`.
- Codex invocation: `$meta-extract-knowledge-extract`.
...
echo '  "project": "'{{SC_ARGS}}'",' >> /tmp/knowledge-extract-state-$SESSION_ID.json
...
- Each sub-agent executes independently using spawn_agent tool
