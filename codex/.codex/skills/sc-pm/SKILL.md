...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-pm`.
- Treat all user text after `$sc-pm` as `{{SC_ARGS}}`.
...
- **Multi-Domain spawn_agents**: Cross-functional coordination requiring multiple specialists
...
5. **Progress Monitoring**: Track execution via update_plan, validate quality gates
...
## Tool Coordination
- **update_plan**: Hierarchical task tracking across all phases
- **spawn_agent**: Advanced delegation for complex multi-agent coordination
