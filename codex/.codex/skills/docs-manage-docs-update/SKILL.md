...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$docs-manage-docs-update`.
- Treat all user text after `$docs-manage-docs-update` as `{{SC_ARGS}}`.
...
- Original command: `docs:manage:docs-update`.
- Codex invocation: `$docs-manage-docs-update`.
...
- DETERMINE update scope based on {{SC_ARGS}}:
...
- USE spawn_agent tool for parallel documentation processing:
