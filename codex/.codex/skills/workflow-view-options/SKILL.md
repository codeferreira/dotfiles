...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$workflow-view-options`.
- Treat all user text after `$workflow-view-options` as `{{SC_ARGS}}`.
...
- Original command: `workflow:view:options`.
- Codex invocation: `$workflow-view-options`.
...
- PARSE problem statement from {{SC_ARGS}}
