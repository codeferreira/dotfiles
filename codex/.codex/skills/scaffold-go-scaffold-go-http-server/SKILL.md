---
name: "scaffold-go-scaffold-go-http-server"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$scaffold-go-scaffold-go-http-server`.
- Treat all user text after `$scaffold-go-scaffold-go-http-server` as `{{SC_ARGS}}`.
...
- Original command: `scaffold:go:scaffold-go-http-server`.
- Codex invocation: `$scaffold-go-scaffold-go-http-server`.
...
- DETERMINE project name: IF {{SC_ARGS}} provided, use it; ELSE use current directory name
