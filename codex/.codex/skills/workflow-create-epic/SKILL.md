...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$workflow-create-epic`.
- Treat all user text after `$workflow-create-epic` as `{{SC_ARGS}}`.
...
- Original command: `workflow:create:epic`.
- Codex invocation: `$workflow-create-epic`.
...
- ANALYZE target scope from {{SC_ARGS}}
