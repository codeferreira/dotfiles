...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-infra-health-check`.
- Treat all user text after `$ops-infra-health-check` as `{{SC_ARGS}}`.
...
- Original command: `ops:infra:health-check`.
- Codex invocation: `$ops-infra-health-check`.
