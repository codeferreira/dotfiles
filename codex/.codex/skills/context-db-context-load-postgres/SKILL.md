---
name: "context-db-context-load-postgres"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$context-db-context-load-postgres`.
- Treat all user text after `$context-db-context-load-postgres` as `{{SC_ARGS}}`.
...
- Original command: `context:db:context-load-postgres`.
- Codex invocation: `$context-db-context-load-postgres`.
