...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$git-resolve-merge-conflicts`.
- Treat all user text after `$git-resolve-merge-conflicts` as `{{SC_ARGS}}`.
...
- Original command: `git:resolve-merge-conflicts`.
- Codex invocation: `$git-resolve-merge-conflicts`.
