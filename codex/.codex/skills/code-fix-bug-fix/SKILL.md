...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$code-fix-bug-fix`.
- Treat all user text after `$code-fix-bug-fix` as `{{SC_ARGS}}`.
...
- Original command: `code:fix:bug-fix`.
- Codex invocation: `$code-fix-bug-fix`.
...
Fix the bug described in: **{{SC_ARGS}}**
