...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$sc-select-tool`.
- Treat all user text after `$sc-select-tool` as `{{SC_ARGS}}`.
...
- Original command: `select-tool`.
- Codex invocation: `$sc-select-tool`.
