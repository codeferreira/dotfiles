...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$git-pr-pr-check`.
- Treat all user text after `$git-pr-pr-check` as `{{SC_ARGS}}`.
...
- Original command: `git:pr:pr-check`.
- Codex invocation: `$git-pr-pr-check`.
...
- PARSE command arguments from {{SC_ARGS}}:
  - IF {{SC_ARGS}} contains PR number: Extract and validate PR exists
...
spawn_agent 1: "CI/CD Health Agent - Analyze all GitHub Actions workflows, checks, and their current status for PR #$PR_NUMBER. Include running, failed, and pending checks with detailed failure analysis."

spawn_agent 2: "Review Status Agent - Examine all code reviews, approval requirements, and reviewer comments for PR #$PR_NUMBER. Identify blocking reviews and required approvals."

spawn_agent 3: "Merge Conflict Agent - Detect merge conflicts, branch protection rules, and merge readiness for PR #$PR_NUMBER. Analyze conflict complexity if present."

spawn_agent 4: "Security & Compliance Agent - Check security scanning results, license compliance, and policy violations for PR #$PR_NUMBER. Include dependency vulnerabilities."

spawn_agent 5: "Performance Impact Agent - Analyze code changes for performance implications, bundle size changes, and resource usage for PR #$PR_NUMBER."

spawn_agent 6: "Test Coverage Agent - Examine test results, coverage reports, and quality gates for PR #$PR_NUMBER. Identify untested code paths."

spawn_agent 7: "Integration Status Agent - Check API compatibility, breaking changes, and downstream impact for PR #$PR_NUMBER. Verify contract testing."

spawn_agent 8: "Documentation Agent - Verify documentation updates, API doc changes, and changelog entries for PR #$PR_NUMBER. Check for missing docs."
```
...
IF CI_SCORE < 70:
  spawn_agent 9: "CI Failure Analyzer - Deep dive into failing checks, extract error logs, and suggest specific fixes for PR #$PR_NUMBER"

IF SECURITY_SCORE < 80:
  spawn_agent 10: "Security Remediation Agent - Provide specific security fixes and vulnerability patches for PR #$PR_NUMBER"

IF MERGE_SCORE < 90:
  spawn_agent 11: "Conflict Resolution Agent - Generate merge conflict resolution strategies and commands for PR #$PR_NUMBER"

IF TEST_SCORE < 75:
  spawn_agent 12: "Test Generation Agent - Identify missing tests and generate test templates for PR #$PR_NUMBER"

IF DOC_SCORE < 80:
  spawn_agent 13: "Documentation Generator - Create missing documentation and API doc updates for PR #$PR_NUMBER"
```
...
**IF {{SC_ARGS}} contains "watch" OR "monitor", DEPLOY continuous monitoring agents:**
