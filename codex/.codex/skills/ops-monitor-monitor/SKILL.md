...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-monitor-monitor`.
- Treat all user text after `$ops-monitor-monitor` as `{{SC_ARGS}}`.
...
- Original command: `ops:monitor:monitor`.
- Codex invocation: `$ops-monitor-monitor`.
...
echo "✅ Monitoring deployment completed for {{SC_ARGS}}"
