...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-ci-ci-gen`.
- Treat all user text after `$ops-ci-ci-gen` as `{{SC_ARGS}}`.
...
- Original command: `ops:ci:ci-gen`.
- Codex invocation: `$ops-ci-ci-gen`.
...
- PARSE {{SC_ARGS}} for platform and template preferences
...
platform=$(echo "{{SC_ARGS}}" | rg --only-matching 'platform=([^\s]+)' -r '$1' 2>/dev/null || echo "github")
template=$(echo "{{SC_ARGS}}" | rg --only-matching 'template=([^\s]+)' -r '$1' 2>/dev/null || echo "standard")
update_mode=$(echo "{{SC_ARGS}}" | rg --quiet 'update' && echo "true" || echo "false")
security_focus=$(echo "{{SC_ARGS}}" | rg --quiet 'security' && echo "true" || echo "false")
```
