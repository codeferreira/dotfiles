---
name: "scaffold-deno-scaffold-deno-fresh"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$scaffold-deno-scaffold-deno-fresh`.
- Treat all user text after `$scaffold-deno-scaffold-deno-fresh` as `{{SC_ARGS}}`.
...
- Original command: `scaffold:deno:scaffold-deno-fresh`.
- Codex invocation: `$scaffold-deno-scaffold-deno-fresh`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Project name: {{SC_ARGS}}
...
- WARN: "Directory '{{SC_ARGS}}' already exists"
- ANALYZE existing structure with modern tools:
  ```bash
  echo "📁 Existing directory contents:"
  eza -la "{{SC_ARGS}}" 2>/dev/null || ls -la "{{SC_ARGS}}"

  if fd "deno.json" "{{SC_ARGS}}" >/dev/null; then
    echo "🦕 Existing Deno project detected"
    bat "{{SC_ARGS}}/deno.json" 2>/dev/null | head -20
  fi
  ```
...
echo "🚀 Initializing Fresh 2.0 project: {{SC_ARGS}}"
deno run -Ar jsr:@fresh/init@2.0.0-alpha.34 "{{SC_ARGS}}"
```
...
echo "✅ Fresh 2.0 project '{{SC_ARGS}}' created successfully"
echo ""
...
echo "   mkdir {{SC_ARGS}} && cd {{SC_ARGS}}"
echo "   deno run -A -r https://fresh.deno.dev ."
echo ""
echo "2. Alternative Initializer:"
echo "   deno run -A jsr:@fresh/init {{SC_ARGS}}"
echo ""
echo "3. Fresh GitHub Template:"
echo "   git clone https://github.com/denoland/fresh-example.git {{SC_ARGS}}"
```
