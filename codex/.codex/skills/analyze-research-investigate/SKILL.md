---
name: "analyze-research-investigate"
...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$analyze-research-investigate`.
- Treat all user text after `$analyze-research-investigate` as `{{SC_ARGS}}`.
...
- Original command: `analyze:research:investigate`.
- Codex invocation: `$analyze-research-investigate`.
...
- Parse investigation topic: {{SC_ARGS}}
...
- Search existing implementations: rg "{{SC_ARGS}}" --type-add 'code:*.{rs,go,java,ts,js,py}' -t code
- Find related configuration: rg "{{SC_ARGS}}" --type yaml --type json --type toml
- Look for documentation: fd "README|ARCHITECTURE|DESIGN" --type f | xargs rg -l "{{SC_ARGS}}"

STEP 3: Pattern Discovery and Architecture Analysis

TRY:

- Find implementation patterns: rg "impl._{{SC_ARGS}}|class.*{{SC_ARGS}}|interface._{{SC_ARGS}}" -A 5 -B 2
- Analyze dependencies: rg "{{SC_ARGS}}" package.json Cargo.toml go.mod pom.xml requirements.txt
- Check test coverage: fd "test|spec" --type f | xargs rg -l "{{SC_ARGS}}"
...
- Search for "{{SC_ARGS}} best practices 2025"
