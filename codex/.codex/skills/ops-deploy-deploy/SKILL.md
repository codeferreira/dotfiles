...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$ops-deploy-deploy`.
- Treat all user text after `$ops-deploy-deploy` as `{{SC_ARGS}}`.
...
- Original command: `ops:deploy:deploy`.
- Codex invocation: `$ops-deploy-deploy`.
- Preserve the behavioral intent of the source command, but use Codex-native tools and collaboration primitives.
</codex_skill_adapter>

## Context

- Target application: {{SC_ARGS}}
