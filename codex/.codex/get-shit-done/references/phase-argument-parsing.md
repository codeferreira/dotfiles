...
```bash
PHASE_INFO=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
