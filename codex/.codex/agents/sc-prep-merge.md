...
IF (the user request.includes("--all-worktrees")) {
  options.allWorktrees = true;
} ELSE IF (the user request.includes("--dry-run")) {
  options.dryRun = true;
} ELSE IF (the user request.includes("--create-prs")) {
  options.createPRs = true;
} ELSE IF (the user request && !the user request.startsWith("--")) {
  targetBranches = the user request.split(" ");
} ELSE {
