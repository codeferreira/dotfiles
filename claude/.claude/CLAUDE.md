...
- **Backend Languages**: Golang (Chi), NodeJS (Kraken, Express), Java (Spring Boot/Quarkus)
- **Scripting**: Golang
- **Web Framework**: NextJS (Typescript), React (Typescript),
...
- Use Chi for backend development
...
All projects MUST use npm as task runner with standardized commands:
...
### NPM Task Configuration

```json
{
  "scripts": {
    "up": "docker compose up -d && cd backend && go run .",
    "down": "docker compose down",
    "test": "npm run test:backend && npm run test:frontend",
    "dev": "docker compose up -d db && concurrently \"cd backend && go run .\" \"cd frontend && npm run dev\""
  }
}
```
